...
import { getNovelMaterials, saveNovelMaterial, deleteNovelMaterial } from "@/lib/api";
...
function MaterialsContent() {
  ...
  useEffect(() => {
    if (!novelId) return;
    getNovelMaterials(novelId).then(setMaterials);
  }, [novelId]);

  async function handleAdd() {
    if (!title.trim() || !content.trim() || !novelId) return;
    setSaving(true);
    try {
      const saved = await saveNovelMaterial(novelId, { title, content, type: "text" });
  ...
  async function handleDelete(id: string) {
    if (!novelId) return;
    await deleteNovelMaterial(novelId, id);
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async (ev) => {
      ...
      const saved = await saveNovelMaterial(novelId, {
        title: file.name,
        content: text,
        type: "file",
        file_type: file.type,
